...
class telescope():
    """
module doc:
    optimus.telescope() will return a string cotain the default details of telescope.
    optimus.telescope([optinal parameters])
    [optinal parameters]: name/f1/f2/nchan/t0/t1/gain/tsys/raj/decj/useAngel/tsamp/digitiser. 
How to change the default parameters: 
    change f1 to 100 MHz ==> optimus.telescope(f1 = 100)
Note:
    input parameters (name) should be string.
    And other paramters should be float.
    f1/f2: MHz; nchan: int; t0/t1: sec; gain/tsys: float;raj/decj: rad; useAngel: 0/1; tsamp: sec; digitiser: 1/2/8
    """
...
class pulsar():
    """
module doc:
    optimus.pulsar() will return a string cotain the default details of pulsar.
    optimus.pulsar([optinal parameters])
    [optinal parameters]: name/period/dm/raj/decj/width/flux/useAngel. 
How to change the default parameters: 
    change period to 0.111 ==> optimus.pulsar(period = '0.111')
Note: 
    input parameters (name, period, dm, flux) should be string.
    And other paramters should be float
    period: sec; dm: cm-3/pc; raj/decj: rad; width: in range(0,1), flux: Jy; useAngel: 0/1
    """
    #'name: J1950+30\n', 'p0: 0.2\n', 'dm: 50.0\n', 'raj: 4.510914803\n', 'decj: 0.13602659 \n', 'width: 0.01\n', 'flux: 0.000008\n', 'useAngle: 0\n'
...
class writeParamFile() :
    """
module doc:
    optimus.writeParamFile() will wirte the input string to a given filename.
    optimus.writeParamFile(filename,strings)
example:
    filename = 'test.params'; strings = 'test string line 1\ntest string line2'
    optimus.writeParamFile(filename,strings)
    """
...
def pulseWidth(period,meanFref,randomNum):
    """
module doc:
    optimus.pulseWidth() will return the width of the pulsar from the input parameters.
    optimus.pulseWidth(period,meanFref,randomNum)
Note: 
    randomNum should be in (-1,1)
    while randomNum is 0, the width will be the max.
    while randomNum is -1/1, the width will be 0.
    input parameters (period,meanFref,randomNum) should be float.
    period: sec, meanFref: MHz
    """
